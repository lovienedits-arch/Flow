...
def _estimate_features_for_route(route, chunk_index=0, scenario_overrides=None):
    ...
    base_vc = random.randint(140, 320)
    base_speed = random.randint(28, 48)
    if 1 <= chunk_index <= 2:
        base_vc += random.randint(20, 80)
        base_speed -= random.randint(2, 8)
    ...
    for route in routes:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if scenario == "rain":
                ...
                feats["vehicle_count"] = random.randint(360, 460)
                feats["average_speed"] = random.randint(18, 26)
            elif scenario == "very_high":
                feats["vehicle_count"] = random.randint(500, 580)
                feats["average_speed"] = random.randint(10, 18)
            elif scenario == "closure":
                feats["vehicle_count"] = random.randint(440, 520)
                feats["average_speed"] = random.randint(12, 20)
            elif scenario == "event":
                feats["vehicle_count"] = random.randint(460, 560)
                feats["average_speed"] = random.randint(14, 22)
