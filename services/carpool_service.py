...
def list_my_offers(user_id):
    ...
    cur = conn.cursor()
    now = int(time.time())
    # Only show real, non-expired offers (departure within last hour or future, and not cancelled)
    # Cleanup old expired offers (>3 hours past) for this user to avoid fake clutter
    cur.execute('DELETE FROM carpool_offers WHERE user_id=? AND departure_time < ? AND status != "open"', (user_id, now - 10800))
    rows = cur.execute('SELECT * FROM carpool_offers WHERE user_id=? AND departure_time > ? AND status IN ("open","full") ORDER BY departure_time ASC LIMIT 20', (user_id, now - 3600)).fetchall()
    # also joined - only upcoming
    joined = cur.execute('''SELECT o.*, u.username as driver_username, u.display_name as driver_display
        ...
        WHERE j.rider_id=? AND j.status='confirmed' AND o.departure_time > ? ORDER BY o.departure_time ASC LIMIT 20''', (user_id, now - 3600)).fetchall()
    conn.commit()
    conn.close()
