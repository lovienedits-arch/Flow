...
def compute_eco_metrics(route, avg_congestion_score, weather=None):
    ...
    # Prefer explicit avg_speed from route intelligence (already speed-corrected)
    avg_speed = route.get("avg_speed")
    if avg_speed is None:
        # Fallback: compute from distance/eta
        eta_h = max(0.1, route.get("eta_minutes",10)/60)
        avg_speed = dist / eta_h if eta_h else 25
