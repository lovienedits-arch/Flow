...
async function initMap() {
  ...
  map.on('load', () => {
    ...
    // Blocked / obstruction visual cues — black/red dashed for blocked, icon for obstructions
    map.addSource('obstruction-points', { type:'geojson', data:{type:'FeatureCollection',features:[]}});
    map.addLayer({ id:'blocked-outline', type:'line', source:'traffic-segments', filter:['==',['get','blocked'],true], layout:{'line-join':'round','line-cap':'round'}, paint:{'line-color':'#ef4444','line-width':10,'line-opacity':0.9}});
    map.addLayer({ id:'blocked-lines', type:'line', source:'traffic-segments', filter:['==',['get','blocked'],true], layout:{'line-join':'round','line-cap':'round'}, paint:{'line-color':'#000000','line-width':8,'line-opacity':1,'line-dasharray':[0.5,2]}});
    map.addLayer({ id:'obstruction-icons', type:'symbol', source:'obstruction-points', layout:{'text-field':'🚧','text-size':18,'text-allow-overlap':true,'text-ignore-placement':true}, paint:{}});
...
function updateObstructions(trafficFC){
  if(!map||!map.getSource('obstruction-points')) return;
  if(!trafficFC || !trafficFC.features){ map.getSource('obstruction-points').setData({type:'FeatureCollection',features:[]}); return; }
  const points=[];
  for(const f of trafficFC.features){
    const props=f.properties;
    if(props.blocked || props.category==='Severe' || (props.category==='High' && props.score>80)){
      const coords=f.geometry.coordinates;
      const mid=coords[Math.floor(coords.length/2)];
      points.push({type:'Feature', geometry:{type:'Point', coordinates:mid}, properties:{category:props.category, blocked:!!props.blocked, road:props.road_name}});
    }
  }
  map.getSource('obstruction-points').setData({type:'FeatureCollection', features: points});
}
