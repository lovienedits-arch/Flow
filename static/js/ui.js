...
const UI = {
  ...
  setupInteractions(){
    ...
    const stopBtn=document.getElementById('btn-stop-nav-top');
    if(stopBtn) stopBtn.onclick=()=> this.stopLiveNavigation();
    ...
    // FlowPool under addresses (2 tabs like login/signup)
    document.querySelectorAll('#flowpool-tabs button').forEach(btn=>{
      btn.onclick=()=>{
        document.querySelectorAll('#flowpool-tabs button').forEach(b=>b.classList.remove('active'));
        btn.classList.add('active');
        const tab=btn.dataset.tab;
        document.getElementById('flowpool-find').classList.toggle('hidden', tab!=='find');
        document.getElementById('flowpool-offer').classList.toggle('hidden', tab!=='offer');
        if(tab==='find') this.refreshFlowPoolFind();
      };
    });
    document.getElementById('btn-flowpool-offer').onclick=()=> this.offerFlowPool();
    const linkSetup=document.getElementById('link-carpool-setup');
    if(linkSetup) linkSetup.onclick=(e)=>{ e.preventDefault(); this.toggleCarpoolPanel(); };
    // Carpool trip toggle under addresses — show FlowPool only if selected, otherwise neat navigation
    const carpoolTripToggle=document.getElementById('carpool-trip-toggle');
    const flowpoolWidget=document.getElementById('flowpool-widget');
    if(carpoolTripToggle && flowpoolWidget){
      // Initially hidden
      flowpoolWidget.classList.add('hidden');
      carpoolTripToggle.onchange=(e)=>{
        flowpoolWidget.classList.toggle('hidden', !e.target.checked);
        if(e.target.checked){
          // Ensure car details are set, prompt if not
          this.loadCarpoolProfile().then(()=>{
            // If profile not okay, show hint
            const chk=document.getElementById('carpool-ok');
            if(chk && !chk.checked){
              toast('Set car details in Carpool panel to host');
            }
          });
          // Switch to offer tab by default when enabling carpool for trip
          document.querySelectorAll('#flowpool-tabs button').forEach(b=>b.classList.remove('active'));
          const offerTab=document.querySelector('#flowpool-tabs button[data-tab="offer"]');
          if(offerTab) offerTab.classList.add('active');
  ...
  async checkAuth(){
    ...
    // Optimistically show logged in from localStorage to avoid flash of modal on reload
    if(savedUser){
      try{
        const u=JSON.parse(savedUser);
        this.updateProfileUI(u);
    ...
    try{
      ...
      if(me.user){
        ...
        localStorage.setItem('flow_user', JSON.stringify(me.user));
        document.getElementById('auth-modal').classList.add('hidden');
      ...
      // Network error - keep optimistic logged in state, don't prompt
      if(savedUser){
        try{
          const u=JSON.parse(savedUser);
          this.updateProfileUI(u);
      ...
      this.updateProfileUI(null);
      // Don't show modal on network error if we have token - assume still logged in
    }
  },
  ...
  async handleLogin(){
    ...
    localStorage.setItem('flow_user', JSON.stringify({username: res.username, display_name: res.display_name, id: res.user_id}));
  ...
  closeAllSidePanels(exceptId){
    ['track-panel','carpool-panel','city-panel'].forEach(id=>{
  ...
  toggleTrackPanel(){
    ...
    this.closeAllSidePanels('track-panel');
    if(isHidden){
  ...
  toggleCarpoolPanel(){
    ...
    const isHidden=p.classList.contains('hidden');
    this.closeAllSidePanels('carpool-panel');
    if(isHidden){
      p.classList.remove('hidden');
      this.loadCarpoolProfile();
  ...
  showCarpoolSuggestion(matches){
    ...
    list.innerHTML = `<div style="font-size:11px;color:var(--text-main);background:var(--bg-soft);border:1px solid var(--border);padding:6px 8px;border-radius:8px;margin-bottom:8px">💚 ${matches.length} going same way • Pool in ${matches[0].departure_in_mins} mins • Save ~${totalCO2} kg CO₂</div>` +
      matches.map(m=>`<div class="carpool-match"><h4 style="color:var(--text-main)">👥 ${m.driver_display} <small style="color:var(--text-muted)">@${m.driver_username}</small> <span class="badge" style="background:#dcfce7;color:#166534">${m.seats_left} seats left</span></h4><div style="font-size:12px;color:var(--text-muted)">${m.origin_name||'Origin'} → ${m.dest_name||'Dest'} • Departs in ${m.departure_in_mins} mins</div><button class="btn-primary small" data-join="${m.offer_id}" style="margin-top:6px">Join FlowPool</button></div>`).join('');
    ...
    // Also sync to FlowPool widget (simplified)
    const fpList=document.getElementById('flowpool-find-list');
    const fpStatus=document.getElementById('flowpool-find-status');
    if(fpList && fpStatus){
      if(matches && matches.length){
        fpStatus.textContent=`${matches.length} pools on this corridor — tap Join`;
        fpList.innerHTML = matches.map(m=>`<div class="flowpool-card" style="color:var(--text-main)"><strong style="color:var(--text-main)">${m.driver_display} <small style="color:var(--text-muted)">@${m.driver_username}</small></strong> <span class="badge" style="background:#dcfce7;color:#166534">${m.seats_left} left</span><br><small style="color:var(--text-muted)">${m.origin_name||'Origin'} → ${m.dest_name||'Dest'} • in ${m.departure_in_mins} mins</small><br><button class="btn-primary small" data-fpjoin="${m.offer_id}" style="margin-top:4px">Join FlowPool</button></div>`).join('');
        fpList.querySelectorAll('[data-fpjoin]').forEach(b=>{
          b.onclick=async()=>{
            const r=await API.carpoolJoin(b.dataset.fpjoin);
            if(r.error) toast(r.error); else { toast('Joined FlowPool!'); this.refreshFlowPoolFind(); this.refreshCarpoolOffers(); }
          };
        });
      }
    }
  },
  async refreshFlowPoolFind(){
    const list=document.getElementById('flowpool-find-list');
    const status=document.getElementById('flowpool-find-status');
    if(!list || !status) return;
    if(!currentOriginCoords || !currentDestCoords){
      status.textContent='Enter origin & destination then Navigate to see pools on this corridor.';
      list.innerHTML='';
      return;
    }
    status.textContent='Searching FlowPools on this route…';
    try{
      const res=await API.carpoolSearch(currentOriginCoords, currentDestCoords);
      if(!res.matches || !res.matches.length){
        status.textContent='No pools yet on this corridor — be first to host!';
        list.innerHTML='';
        return;
      }
      status.textContent=`${res.matches.length} pools found • in ${res.matches[0].departure_in_mins} mins`;
      list.innerHTML=res.matches.map(m=>`<div class="flowpool-card" style="color:var(--text-main)"><strong style="color:var(--text-main)">${m.driver_display} <small style="color:var(--text-muted)">@${m.driver_username}</small></strong> <span class="badge" style="background:#dcfce7;color:#166534">${m.seats_left} left</span><br><small style="color:var(--text-muted)">${m.origin_name||'Origin'} → ${m.dest_name||'Dest'} • in ${m.departure_in_mins} mins</small><br><button class="btn-primary small" data-fpjoin="${m.offer_id}" style="margin-top:4px">Join FlowPool</button></div>`).join('');
      list.querySelectorAll('[data-fpjoin]').forEach(b=>{
        b.onclick=async()=>{
          const r=await API.carpoolJoin(b.dataset.fpjoin);
          if(r.error) toast(r.error); else { toast('Joined FlowPool!'); this.refreshFlowPoolFind(); this.refreshCarpoolOffers(); }
        };
  ...
  async offerFlowPool(){
    if(!localStorage.getItem('flow_token')){ toast('Log in to host FlowPool'); this.showAuth(); return; }
    if(!currentOriginCoords || !currentDestCoords){ toast('Set origin & destination first'); return; }
    const prof=await API.carpoolProfile();
    if(!prof.okay_with_carpool){ toast('Enable carpool in settings first'); this.toggleCarpoolPanel(); return; }
    const sel=document.getElementById('flowpool-depart');
    const mins=parseInt(sel ? sel.value : '30');
    const dep=Math.floor(Date.now()/1000)+mins*60;
    const originName=document.getElementById('origin-input').value||'Origin';
    const destName=document.getElementById('dest-input').value||'Destination';
    const res=await API.createCarpoolOffer({origin: currentOriginCoords, dest: currentDestCoords, origin_name: originName, dest_name: destName, departure_time: dep});
    const statusEl=document.getElementById('flowpool-offer-status');
    if(res.error){ if(statusEl) statusEl.textContent=res.error; toast(res.error); }
    else { if(statusEl) statusEl.textContent='FlowPool hosted for '+mins+' mins — others will see it!'; toast('FlowPool hosted ✓'); this.refreshCarpoolOffers(); this.refreshFlowPoolFind(); }
  },

  startLiveNavigation(){
    if(!currentDestCoords){ toast('Set destination first'); return; }
    if(!navigator.geolocation){ toast('Live navigation not supported on this browser'); return; }
    if(window.isSecureContext===false){
      toast('Live GPS needs HTTPS — try https://localhost or use map tap. Attempting anyway…');
    }
    if(window.isNavigating){ this.stopLiveNavigation(); return; }

    const startBtn=document.getElementById('btn-start-nav');
    if(startBtn){ startBtn.textContent='Locating…'; startBtn.disabled=true; }

    const startNav = ()=>{
      window.isNavigating=true;
      if(startBtn){ startBtn.textContent='● Navigating — Tap to stop'; startBtn.style.background='#16a34a'; startBtn.disabled=false; }
      const navPanel=document.getElementById('nav-panel');
      if(navPanel) navPanel.classList.remove('hidden');
      document.getElementById('nav-status').textContent='Acquiring GPS…';
      toast('Live navigation started — blue dot follows you');

      let currentSteps = [];
      let currentRoute = null;
      if(this.lastRoutes && this.lastRoutes.length){
        currentRoute = this.lastRoutes.find(r=>r.is_recommended)||this.lastRoutes[0];
        if(currentRoute.legs && currentRoute.legs.length){
          currentSteps=[];
          currentRoute.legs.forEach(leg=>{
            (leg.steps||[]).forEach(s=>{
              currentSteps.push({
                instruction: s.maneuver?.instruction || (s.maneuver?.type + (s.name? ' onto '+s.name:'')) || s.name || 'Continue',
                name: s.name||'',
        ...
        } else if(currentRoute.steps && currentRoute.steps.length){
          currentSteps=currentRoute.steps.map(s=>({
        ...
        if(currentSteps.length){
          this._navSteps=currentSteps;
          this._navStepIndex=0;
          this._renderNavSteps();
        }
      }
      ...
      const onPos=(pos)=>{
        ...
        if(map){
          map.panTo([lon,lat], {duration:700});
          if(heading!==null && !isNaN(heading) && typeof map.setBearing==='function'){
            try{ map.easeTo({bearing:heading, duration:700, pitch:45}); }catch{}
          }
        }
        document.getElementById('nav-status').textContent='GPS live • ' + (speed? (speed*3.6).toFixed(0)+' km/h • ':'') + new Date().toLocaleTimeString();
        const dest=currentDestCoords;
        if(dest){
          const remainingStraight = this._haversine(lon, lat, dest[0], dest[1]);
          let remainingKm = remainingStraight;
          let etaMins = 0;
          if(currentRoute){
            const avgSpeed = currentRoute.avg_speed || currentRoute.eco?.avg_speed_kmh || 28;
            etaMins = Math.round(remainingKm / Math.max(8, avgSpeed) * 60);
          } else {
            etaMins = Math.round(remainingKm / 22 * 60);
          }
          const elDist=document.getElementById('route-distance');
          if(elDist) elDist.textContent=`${remainingKm.toFixed(1)} km remaining • Live`;
          const elEta=document.getElementById('route-eta');
          if(elEta) elEta.textContent=formatDuration(etaMins);
          document.getElementById('nav-remaining').textContent=`Remaining: ${remainingKm.toFixed(1)} km • ${formatDuration(etaMins)} • ${currentRoute? currentRoute.distance_km+' km total':''}`;
          if(this._navSteps && this._navSteps.length){
            const next = this._navSteps[this._navStepIndex] || this._navSteps[0];
            if(next){
              document.getElementById('nav-next-maneuver').textContent=next.instruction || next.name || 'Continue';
              document.getElementById('nav-next-distance').textContent=`${(next.distance/1000).toFixed(1)} km • ${(next.duration/60).toFixed(0)} min • ${next.name||''}`;
              this._highlightNavStep(this._navStepIndex);
              if(remainingStraight < 0.04 && this._navStepIndex < this._navSteps.length-1){
                this._navStepIndex++;
              }
            }
          } else {
            document.getElementById('nav-next-maneuver').textContent='Head to destination';
            document.getElementById('nav-next-distance').textContent=`${remainingKm.toFixed(1)} km straight`;
          }
          ...
          if(currentRoute && currentRoute.geometry && currentRoute.geometry.coordinates){
            const distToRoute = this._distanceToRoute(lon, lat, currentRoute.geometry.coordinates);
            if(distToRoute > 0.06){
              document.getElementById('nav-status').textContent='Off route — rerouting…';
              shouldReroute=true;
            }
          }
          if(shouldReroute){
            ...
            if(!this._lastReroute || Date.now()-this._lastReroute>10000){
              this._lastReroute=Date.now();
              document.getElementById('nav-status').textContent='Rerouting…';
              API.getRoute([lon,lat], dest, {scenario:this.currentScenario, horizon:this.currentHorizon}).then(data=>{
                if(data.routes && data.routes.length){
                  ...
                  if(rec.legs){
                    let newSteps=[];
                    rec.legs.forEach(leg=> (leg.steps||[]).forEach(s=> newSteps.push({instruction:s.maneuver?.instruction||s.name||'Continue', name:s.name||'', distance:s.distance||0, duration:s.duration||0, maneuver:s.maneuver||{}})));
                    this._navSteps=newSteps;
                    this._navStepIndex=0;
                    this._renderNavSteps();
                  }
                  ...
                  document.getElementById('nav-status').textContent='Rerouted • following live';
                }
              }).catch(()=>{ document.getElementById('nav-status').textContent='Reroute failed — check connection'; });
            }
          }
        }
      };
      const onErr=(err)=>{
        if(startBtn){ startBtn.textContent='Start'; startBtn.style.background=''; startBtn.disabled=false; }
        document.getElementById('nav-status').textContent='GPS error';
        let msg=err.message||'';
        if(msg.includes('Only secure origins') || !window.isSecureContext){
          toast('Live GPS blocked: use HTTPS/localhost — showing static route');
          document.getElementById('nav-next-maneuver').textContent='GPS blocked — using static route';
        } else if(err.code===1){
          toast('Location permission denied — please allow to navigate');
          document.getElementById('nav-next-maneuver').textContent='Permission denied';
        } else if(err.code===2){
          toast('GPS unavailable — check signal');
        } else if(err.code===3){
          toast('GPS timeout — retrying');
          setTimeout(()=>{ if(window.isNavigating) navigator.geolocation.getCurrentPosition(onPos, onErr, {enableHighAccuracy:true, timeout:8000}); }, 1500);
          return;
        } else {
          toast('Navigation error: '+msg);
        }
      };
      navigator.geolocation.getCurrentPosition((pos)=>{
        onPos(pos);
        window.liveNavWatch=navigator.geolocation.watchPosition(onPos, onErr, {enableHighAccuracy:true, maximumAge:1500, timeout:10000});
      }, onErr, {enableHighAccuracy:true, timeout:10000, maximumAge:0});
  ...
  stopLiveNavigation(){
    ...
    if(btn){ btn.textContent='Start'; btn.style.background=''; btn.disabled=false; }
  ...
  _renderNavSteps(){
    const cont=document.getElementById('nav-steps-list');
    if(!cont || !this._navSteps) return;
    cont.innerHTML=this._navSteps.slice(0,12).map((s,i)=>`<div class="nav-step ${i===this._navStepIndex?'active':''}" data-idx="${i}"><span style="min-width:28px;text-align:center">${i===0?'●':(i+1)}</span><div style="flex:1;min-width:0"><div style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${s.instruction}</div><small>${s.name? s.name+' • ':''}${(s.distance/1000).toFixed(1)} km</small></div></div>`).join('');
  },
  _highlightNavStep(idx){
    document.querySelectorAll('#nav-steps-list .nav-step').forEach(el=> el.classList.toggle('active', parseInt(el.dataset.idx)===idx));
  },
  _distanceToRoute(lon, lat, coords){
    let min=Infinity;
    for(let i=0;i<coords.length-1;i++){
      const a=coords[i], b=coords[i+1];
      const d1=this._haversine(lon,lat,a[0],a[1]);
      const d2=this._haversine(lon,lat,b[0],b[1]);
  ...
  async setMode(mode, btn){
    ...
    btn.classList.add('active');
    if(this.isCityMode){
      this.closeAllSidePanels('city-panel');
  ...
  async refreshTrafficForGeometry(geometry){
    ...
    window._lastTraffic=fc; this.lastTraffic=fc;
    if(map && map.getSource('traffic-segments')) map.getSource('traffic-segments').setData(fc);
    if(typeof updateObstructions==='function') updateObstructions(fc);
