...
const UI = {
  ...
  showCarpoolSuggestion(matches){
    ...
    // Also sync to FlowPool widget
    const fpList=document.getElementById('flowpool-find-list');
    const fpStatus=document.getElementById('flowpool-find-status');
    if(fpList && fpStatus){
      if(matches && matches.length){
        fpStatus.textContent=`${matches.length} pools on this corridor — also shown above`;
        fpList.innerHTML = matches.map(m=>`<div class="flowpool-card"><strong>${m.driver_display} @${m.driver_username}</strong> • ${m.car_model} • ${m.seats_left} seats<br><small>${m.origin_name||'Origin'} → ${m.dest_name||'Dest'} • in ${m.departure_in_mins} mins • saves ${m.co2_saved_kg}kg CO₂</small><br><button class="btn-primary small" data-fpjoin="${m.offer_id}" style="margin-top:4px">Join FlowPool</button></div>`).join('');
  ...
  async refreshFlowPoolFind(){
    ...
    try{
      ...
      status.textContent=`${res.matches.length} pools found • joining saves emissions, pooled by ${res.matches[0].departure_in_mins} mins`;
      list.innerHTML=res.matches.map(m=>`<div class="flowpool-card"><strong>${m.driver_display} @${m.driver_username}</strong> <span class="badge" style="background:#dcfce7;color:#166534">${m.seats_left} left</span><br><small>${m.origin_name||'Origin'} → ${m.dest_name||'Dest'} • ${m.route_km}km • ${m.car_model} ${m.car_number? '('+m.car_number+')':''}</small><br><small>Departs in ${m.departure_in_mins} mins • saves ${m.co2_saved_kg}kg CO₂</small><br><button class="btn-primary small" data-fpjoin="${m.offer_id}" style="margin-top:4px">Join FlowPool</button></div>`).join('');
