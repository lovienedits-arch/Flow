...
def api_geocode():
    ...
    if local_matches:
        # Always also try to add Nominatim results for richer choices, but prioritize local
        payload = {"results": local_matches[:5]}
        GEOCODE_CACHE[cache_key] = payload
        GEOCODE_TS[cache_key] = now
        # Try to supplement async? For now return local
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "FLOW/1.0 (hackathon@flow.city)"},
            timeout=4
        )
        if resp.status_code == 200:
            ...
            for r in data:
                try:
                    ...
                    continue
            # If no results and query looks like lat,lon? try to parse
            if not results:
                # Allow raw "12.97,77.59" etc
                try:
                    if "," in q:
                        parts = q.split(",")
                        lat = float(parts[0].strip()); lon = float(parts[1].strip())
                        results = [{"name": f"{lat},{lon}", "display_name": f"{lat},{lon} (coordinates)", "coordinates": [lon, lat]}]
                except:
                    pass
