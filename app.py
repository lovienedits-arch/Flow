...
from services.carpool_service import get_carpool_profile, set_carpool_profile, create_offer, list_my_offers, find_matches_for_route, join_offer, cancel_join
...
# ---------- Carpool ----------
@app.route("/api/carpool/profile", methods=["GET", "POST"])
def api_carpool_profile():
    if not g.current_user:
        return jsonify({"error": "Login required"}), 401
    if request.method == "GET":
        prof = get_carpool_profile(g.current_user["id"])
        return jsonify(prof)
    data = request.get_json(force=True) or {}
    okay = bool(data.get("okay_with_carpool"))
    car_model = (data.get("car_model") or "").strip()[:40]
    car_number = (data.get("car_number") or "").strip()[:20]
    seats = int(data.get("seats_available") or 3)
    seats = max(1, min(7, seats))
    fuel = (data.get("fuel_type") or "Petrol").strip()[:20]
    prof = set_carpool_profile(g.current_user["id"], okay, car_model, car_number, seats, fuel)
    return jsonify(prof)

@app.route("/api/carpool/offer", methods=["POST"])
def api_carpool_offer():
    if not g.current_user:
        return jsonify({"error": "Login required"}), 401
    data = request.get_json(force=True) or {}
    origin = data.get("origin")
    dest = data.get("dest")
    # Allow origin/dest as [lon,lat] or use current search from route
    if not origin or not dest:
        return jsonify({"error": "origin and dest required as [lon,lat]"}), 400
    origin_name = data.get("origin_name") or ""
    dest_name = data.get("dest_name") or ""
    # departure_time can be ISO string or epoch; expect epoch or "in 30 mins"
    dep = data.get("departure_time")
    if isinstance(dep, str) and dep.isdigit():
        dep = int(dep)
    elif isinstance(dep, str):
        # try parse "15" mins from now
        try:
            mins = int(dep)
            dep = int(time.time()) + mins*60
        except:
            dep = None
    seats = data.get("seats_total")
    res, err = create_offer(g.current_user["id"], origin, dest, origin_name, dest_name, dep, seats)
    if err:
        return jsonify({"error": err}), 400
    return jsonify(res)

@app.route("/api/carpool/offers", methods=["GET"])
def api_carpool_offers():
    if not g.current_user:
        return jsonify({"error": "Login required"}), 401
    data = list_my_offers(g.current_user["id"])
    return jsonify(data)

@app.route("/api/carpool/search", methods=["POST"])
def api_carpool_search():
    data = request.get_json(force=True) or {}
    origin = data.get("origin")
    dest = data.get("dest")
    if not origin or not dest:
        return jsonify({"error": "origin/dest required"}), 400
    exclude = g.current_user["id"] if g.current_user else None
    matches = find_matches_for_route(origin, dest, departure_within_mins=int(data.get("within_mins") or 45), exclude_user_id=exclude)
    return jsonify({"matches": matches, "count": len(matches)})

@app.route("/api/carpool/join", methods=["POST"])
def api_carpool_join():
    if not g.current_user:
        return jsonify({"error": "Login required"}), 401
    data = request.get_json(force=True) or {}
    ...
    res, err = join_offer(g.current_user["id"], oid)
    if err:
        return jsonify({"error": err}), 400
    return jsonify(res)

@app.route("/api/carpool/leave", methods=["POST"])
def api_carpool_leave():
    if not g.current_user:
        return jsonify({"error": "Login required"}), 401
    data = request.get_json(force=True) or {}
    oid = data.get("offer_id")
    ...
    try:
        ...
        return jsonify({"error": "invalid offer_id"}), 400
    res, err = cancel_join(g.current_user["id"], oid)
    if err:
        return jsonify({"error": err}), 400
    return jsonify(res)
...
def get_routes():
    ...
    try:
        ...
        # Carpool: find others going same corridor within ~45 mins
        try:
            carpool_matches = find_matches_for_route(origin, destination, departure_within_mins=45, exclude_user_id=g.current_user["id"] if g.current_user else None)
        except:
            carpool_matches = []
        result = {"routes": enriched, "weather": weather, "carpool": carpool_matches, "carpool_count": len(carpool_matches)}
        if warning:
